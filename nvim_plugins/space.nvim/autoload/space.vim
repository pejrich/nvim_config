...
function! s:on_leading_or_trailing_whitespace()
  return strpart(getline('.'), 0, col('.')) =~# '^\s\+$' || strpart(getline('.'), col('.')-1) =~# '^\s\+$'
endfunction
function! s:on_single_whitespace()
  return strpart(getline('.'), col('.')-2, 3) =~# '\v^[^ ] [^ ]$'
endfunction
function! s:on_whitespace()
  return strpart(getline('.'), col('.')-2, 3) =~# '\s\s'
  " return strpart(getline('.'), col('.')-1, 1) =~# '\s'
endfunction
...
function! s:around_characterwise()
  if s:on_single_whitespace()
    norm! v
  elseif s:on_leading_or_trailing_whitespace() || s:on_whitespace()
    call s:select_whitespace('all')
  else " not positioned on whitespace
    let next_multiple_spaces = s:next_regex_in_line('\s\s')
    if next_multiple_spaces
