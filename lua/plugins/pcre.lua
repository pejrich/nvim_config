...
local function perform_sub(string, sub_exp)
    local new_lines =
        vim.fn.system(
        "perl -CSDA -e 'use utf8;' -e '#line 1 \"PerlSubstitute\"' -pe " .. vim.fn.shellescape(vim.fn.escape(sub_exp, "%!") .. ";"), string)
    return new_lines
end
...
local function perl_sub_preview(opts, preview_ns, preview_buf)
    ...
    for i, line in ipairs(lines) do
        ...
        if string.find(flags, "g") then
            ...
            line .. '" =~ /' .. lhs .. "/" .. flags .. ') { push @ret, \\$-[0] . "," . \\$+[0]; }; print join "|", @ret;'
        else
        ...
        for x, y in string.gmatch(indexString, "(%d),(%d)(%|-)") do
            ...
            if startidx then
                ...
                if preview_buf then
                    ...
                    vim.api.nvim_buf_add_highlight(preview_buf, preview_ns, "Substitute", preview_buf_line, #prefix + startidx - 1,
                        #prefix + endidx)
                end
            end
